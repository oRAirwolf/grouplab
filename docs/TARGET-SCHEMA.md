...
43a. The pass criterion of test 43 is the **worst bull-centre error on the sheet**, not the residual over marker corners. The corner residual is reported as RMS with the maximum alongside, as a diagnostic of the detector rather than of the definition. DESIGN.md section 21 gives the reasoning, the short form of which is that the corner residual halves when the render goes from 300 to 600 DPI and the bull error does not, so the two are measuring different things and only one of them is what a shooter's group depends on.
