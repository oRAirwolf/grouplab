...
- **Section 3.1 and 3.2, done.** The zero correction in MOA, mil, inches and centimetres at once, the scope's unit leading where the rifle records one and the clicks spelled out, never guessed; and a metric reading of every figure that never touches what is stored.
- **Section 6.1, done.** Mean radius per 100 yards on a scale, with the marks attributed to the podcast they came from at every shot count, and a five shot group told plainly that it could land anywhere across several of them.
- **Section 2, done as a model.** `ShotEditor`: the popover's contents, its actions, its keyboard map and what each one does to the marking. A flyer is kept in the group and an exclusion is not, and a test measures a group before and after a flyer is called to hold that. An arrow key moves a hundredth of an inch on the paper rather than a pixel. A hole size set by hand is kept beside what the detector measured, not over it.
- **Section 5, done as a model.** `AnalysisPanel`: the four blocks, one headline figure each, every figure carrying its explanation and its More link, and a figure too small a group to quote keeping its row with the reason in it. **No number can reach the panel without an explanation behind its ?**, held by a test rather than by a promise.
- **Section 7, records done, screen not.** The three record types filled out as section 7 describes them, with one field list the form and the autocomplete both read, autocomplete matching anywhere in a value, and a name already used refused rather than silently overwriting. **It found a defect:** the record book had never saved a rifle's sight height, zero distance or twist, or any of a load's velocity, coefficient, drag model and bullet figures. Typed into the ballistics page, used, and dropped at the next save with nothing said. Fixed, with a round-trip test.
- **Section 3.3, done.** The shot distance's unit is a real choice beside the number rather than a label following the Settings, and changing it rewrites the number rather than reading the old one as the new unit.
- **Section 6.3, done as a model.** GroupLab reads a likely calibre from the holes it measured, offers it with a sentence saying what it cannot know, and holds Accept on a sheet of bulls until the calibre is answered. Tonight's scan re-run is the argument: naming the calibre is worth five holes on scan 4 and the edge shot on scan 6.
- **Not done: sections 1, 6.2, 7's screen, 8 and 10.** The mockups and the before and after renders, the zero offset picture, the Equipment screen itself with the removal of the shared "rounds or components" box, the ballistics page and the comparison screen. None was started, so none is half built.
- **What the next session should know:** every model the screens need now exists and is tested, so the remaining sections are drawing rather than deciding. Section 1's checklist is still the part to do first, and the before renders it asks for do not exist yet for any screen except settings and library.
- `docs/PHASE1-RESULTS.md` "Entries 131, 132 and 133".
...
- **Item 5, done:** questions 30 and 31 closed as option A. The two ranks a third train would need are written into `docs/UPDATES.md` rather than built, because a rank nothing uses is a rank nothing tests.
- **Item 2b.2, done, and it is the one that mattered most tonight.** A shortfall was silent because the count check only ran when somebody had typed a number, and nobody had, because the sheet already said it. One shot to a bull across fifteen scoring bulls is a statement that fifteen were fired. The count now comes from the sheet's own arithmetic, and the shortfall names the bulls with nothing on them. Nearest-bull still claims no count, because that is the person saying they are not counting.
- **Item 2b.3, done:** the size gate follows the calibre. It was a fixed 0.15 in and refused five real .22 LR holes of 0.11 to 0.14 in. The mistake was assuming a hole is about as wide as the bullet; paper closes behind a bullet, and the smallest of those five was 0.49 of its calibre. The floor is now 0.45 of a named calibre, never below 0.06 in.
- **Item 3.1, done:** `ImpactOffsets` finds one translation per subgroup before any hole is assigned. Three things took a failing test each: it re-centres on the median rather than the mean, so one flyer cannot drag the point of impact and turn one bad shot into a whole bad group; it considers only the bulls the shooter says they aimed at, without which a twenty five bull sheet has a translation for almost any answer; and certainty needs a real gap in dmm as well as in proportion, because two readings that both fit perfectly both cost nothing, and nothing is eighty percent of nothing.
- **Item 3.4, done** as part of it: the median is what makes a hole no offset explains cost nothing.
- **Item 3.3, done in Core:** `AssignmentCertainties` says whether the figures rest on something nobody has confirmed, from contested shots and from an offset that did not settle. The zero correction refuses rather than qualifies, because it is the one figure a person acts on by turning a turret. **Not wired into `GroupFigures` or the screen yet**, which is the rest of 3.3.
- **Item 4.1, done:** the scan's stated resolution offered with the number shown, never applied by itself. A photograph is never offered one, and a stretched scan says so instead of offering a number that is right across and wrong down.
- **Item 2.1, done:** CI parses every PHP file on the Linux runner.
- **Item 1, not done:** it needs a nightly that publishes. Tonight's 403 was diagnosed and fixed (see entry 123 section 1 above), and the freshness check was proved working on run 35591532352, which skipped cleanly instead of failing. The first nightly to publish after that fix is the one the test needs, and it had not published when the night ended.
- **Item 4.2, not done, deliberately: question 34.** Pooling is not plumbing. A pooled group has no single centre, and the three defensible choices measure different things. Building it with a centre chosen by me would put figures in the record book that are not comparable with the ones after the choice is made.
- **Item 2b.6, done 2026-09-22.** All six scans re-run, read only. **Three of the four missed holes are back**, two of them only when the calibre is named: scan 5 goes 18 to 20 and scan 6 goes 9 to 10, both exactly Alan's own counts, and scan 4 goes 19 to 24 against his 23, which is one too many and needs the bull-by-bull look. **Scan 1 bull 2 is still missed**, with a calibre and without, so item 2b.1 is confirmed unfixed and is still the priority. The gate record is unchanged: scan 3 reads 25 holes on 25 bulls, mean radius 0.232 in. This closes the measurement question 35 was waiting on: the narrowed position rule reintroduced no false hole on any of the six.
- **Item 3.5, done 2026-09-22, and it found something.** Scans 4, 5 and 6 assigned with the calibre named. **The sheet-wide offset solver of item 3.1 is built, tested and connected to nothing**: `ImpactOffsets.Solve` is called by no part of the assignment path, because item 3.2 was the item that would have wired it in and it was not done. So all three scans assign exactly as entry 120 found them, and scan 5 reports a mean radius of 1.120 in and an extreme spread of 5.304 in, which are the numbers of a group measured from the wrong centres. **Item 3.2 is now the item that matters most in this entry.**
- **Item 7, done 2026-09-22.** Both guides brought up to tonight's state. No screenshots regenerated: no screen changed, only the models behind them.
- **Items 2 (apart from 2.1, which was already done), 2b.1, 2b.4 in part, 2b.5 in part, 2c, 3.2, 6 and 6b, not done:**
- `docs/PHASE1-RESULTS.md` "Entry 130".
...
- **That link is a finding.** Entry 121 section 2.4 said nothing in the repository may point at `releases/latest`, and the test written for it checked the README alone, so `docs/TESTING-GUIDE.md` had been sending testers to a page that returns nothing ever since. The new builder check caught it on its first run. `ReleaseAssetTests` now covers both guides too.
- **Section 1.2:** every check kept, and two added: a link to `releases/latest` or `v0.1.0` fails the build, and so does a Download page whose buttons do not point at `releases/download/nightly/<stable name>`.
- **Section 1.3:** the four donor PDFs are in `website/donor/`. Hashes in `docs/PHASE1-RESULTS.md`. **They carry nothing private:** no upload address, no email, no path, no credential word, and the only URL in any of them is the DejaVu font licence inside the embedded font. That also answers entry 129 section 6.3 early: the donor instructions PDF does not name the old address, so it does not need regenerating for that reason.
- **Section 1.4:** every asset URL carries a content hash, not only the stylesheet and the script. The hand-rolled `ASSET_VERSION` is gone, because there is nothing left for it to drift from.
- **Section 1.5:** every page carries `<meta name="grouplab-site-build">`, and the publish workflow fails if any page does not carry the commit being published.
- **Section 1.6:** the support page and the application already agree, from entry 126.
- **Section 2:** `ci.yml` has a `website` job that builds on every push and publishes nothing.
- **Section 3:** `.github/workflows/website.yml`. One trigger, `workflow_dispatch`, with a required `reason`. It signs the archive with the existing update signing key and **verifies that signature against the public half compiled into `UpdateKeys.cs` before publishing anything**, so a key that cannot verify its own signature fails in the workflow rather than silently on the server. Four tests hold section 3.1 and 3.6: the only trigger is `workflow_dispatch`, no other workflow calls or dispatches it, nothing else writes to the `site` release, and the nightly's cleanup pattern cannot match the tag `site` (checked by running the pattern, not by reading it).
- **Section 4:** `website/server/` has the sync script, the service, the timer and the installer. Everything the section lists is there, including the rollback when the post-install check fails, and the refusal to resume into anything part-finished. Seven tests drive the refusals with no network and no root, against the script's own functions: three hostile paths, a symlink, a build missing a required page, a build whose pages do not say what they are, and a whole site that is accepted so the checks are not simply refusing everything. Two more hold what it must never do and what the unit lets it write to.
- **Section 5, not done:** the install needs SSH, which Alan approves per command. `website/server/install.py --dry-run` is ready to run and says exactly what it would do.
- **Section 6, not done:** the first publish follows the install.
- **Section 7:** `CLAUDE.md` gains when to publish, the steps of a publish, what to do when it fails, and what never to do.
- **Section 8:** `docs/WEBSITE.md`, including the note that the server's existing backup and offsite scripts cover pissinhot.com only and what should be added to them when they are next touched. Nothing of theirs was changed.
- `docs/PHASE1-RESULTS.md` "Entry 128".
...
- **Section 1:** Phase 9, Performance, in `DESIGN.md` section 21 and the README's Planned section, after the phases that exist, saying it may run alongside Phase 6 and why, and that it starts only when the application works as intended.
- **Section 2:** the gate is not written, and the document says so rather than inventing a number. Its shape is there in the terms a person waits in, per platform, with responsiveness separate from speed and memory beside the times.
- **Sections 3 and 4:** `grouplab bench`, which takes no file from anybody: every case runs against material committed here or generated by GroupLab from its own sheet, so it runs on a bare checkout and on CI. Stage timings come from the stage record rather than a second timing path. The record is `docs/PERFORMANCE.md`, written in place by the command, naming the date, the machine, the build and the commit. CI runs the benchmark and does not gate it.
- **Section 3a:** `BenchCoverageTests` enumerates every class in GroupLab that does work and fails when one has no bench case. On its first run it named 25, which became five new cases and a set of names folded into the cases that already exercise them; three are excluded by name with a reason each.
- **Section 3b:** the controls are found by walking every screen rather than from a list, and each is timed from the click to responsive, to settled and to final paint, with the layout passes it caused and whether it ran on the interface thread. A control that is neither measured nor excluded by name fails a test, and so does an exclusion no screen has.
- **Section 5:** nothing guesses where the slowness is. Every operation section 2 names was measured and the candidates are what the measurements name.
- `docs/PHASE1-RESULTS.md` "Entry 117". After the entries already queued. **This entry adds a phase to the plan and builds the measuring tools. It does not optimise anything yet.**

**Alan's decision:** "Once the program is working as intended, I want to add an optimization phase to work on making the software perform better. I can already tell it is somewhat slow running. This can be a final phase or worked on alongside android builds."

### 1. Where it goes in the plan

- **A new phase in `DESIGN.md` section 21 and the README's Planned section: Performance.** Number it after the existing phases.
- **It may run alongside Phase 6, Android**, and the phase line says so. Android is the reason it matters: a phone is several times slower than Alan's desktop, and an analysis that is merely slow on Windows is unusable there.
- **It starts only when the application works as intended.** Correctness first: a fast wrong answer is worthless, and this project's whole argument is that its numbers can be trusted.

### 2. The gate, which cannot be written yet

**A gate needs a number, and we do not have one.** Entry 115 section 5 asks for the first measurement: how long an analysis takes on a 600 dpi scan and on a phone photograph. **That measurement is this phase's baseline, and the gate is written from it**, not from a figure anybody guessed.

**The gate's shape, so the measurement is taken in the right terms:**
- **The times a person actually waits:** the application's start-up to a usable window; opening an image; a full analysis of a 600 dpi Letter scan; a full analysis of a phone photograph; the analysis screen appearing after Accept; a batch of ten sheets.
- **Per platform**, because the answer differs: Alan's desktop today, and a mid-range Android phone when Phase 6 exists.
- **Responsiveness, separately from speed.** A window that freezes for four seconds feels worse than one that works for six and says what it is doing. The stage timeline already reports progress; the phase should hold the interface to never blocking on the analysis.
- **Memory too**, since a 600 dpi colour scan is about 25 MB before anything is derived from it, and a phone has far less room than a desktop.

### 3. The method, which matters more than any single optimisation

- **Measure first, always.** No change is made because something looks slow. `grouplab bench` runs the standard set of operations and prints a table, and its output is committed as a record in the manner of the Phase 0 gate record, so a change's effect is a difference between two committed tables rather than an opinion.
- **CI reports it, and does not gate it.** Runner speed varies between runs and between machines, so the committed record is measured on Alan's desktop, and the CI figure is printed for information, exactly as the raw records are today.
- **The Phase 0 gate record is the safety rail, and this is the best thing about optimising this project.** Every gate table must stay identical through every optimisation. A faster routine that moves the last digit of a measurement is a defect, not an improvement, and the record catches it on the same push. **Any optimisation that does change a figure has to be argued as a correction, with the reasoning in the record, the way entry 101's convergence fix was.**
- **Profile before choosing.** Name where the time actually goes, per stage, before touching anything. Likely candidates are obvious, and obvious candidates are often wrong: the project has already been surprised by a ten-iteration limit inside a native library.
- **Cheap wins are still wins**, and they come first: work done twice, images copied that could be shared, work done at 600 dpi that a smaller image would settle, and anything computed for a screen nobody opened.

### 3a. It measures everything, by itself, and proves it left nothing out

**Alan's requirement: "GroupLab should be able to benchmark itself with every aspect of the software without user input."** That is right, and it needs two things beyond a list of timings.

**It runs with no input of any kind.** `grouplab bench` takes no file from anybody. Every case runs against committed fixtures or material it generates itself: a rendered synthetic sheet, a committed scan, a committed photograph. It can be run on a bare checkout, on CI, and on a machine that has never analysed a target.

**It covers every aspect, and a test proves the list is complete.** A hand-written list of cases rots the moment somebody adds a feature. **So the bench's coverage is checked, in the manner of the README's own tests:** a test enumerates the things that can be measured and fails when one has no bench case. At least these, each timed separately rather than hidden inside a total:

- **Definitions:** reading and validating a GLTD file, deriving the layout, encoding and decoding the binary QR payload.
- **Rendering:** a sheet to PDF, the same sheet drawn for the printer, the icon set.
- **Image handling:** loading and decoding a 600 dpi Letter scan and a phone photograph, and building the 150 dpi proof image.
- **Measurement, stage by stage**, as the stage record already divides it: marker detection, candidate decoding, registration under each model, the render-and-difference pass, hole detection, one-to-one assignment.
- **Statistics, each separately:** sigma and mean radius with intervals, the CEP table, the bootstrap, the shape tests, the flyer calibration, group comparison, hit probability, and the zero correction.
- **The solver:** one trajectory, and a dope table.
- **Storage:** saving a session, reopening one, the JSON export and import, and a query over a database holding a few hundred sessions, which it generates itself.
- **Documents:** the report PDF, the user guide, the volunteer pack.
- **The interface, driven headlessly**, the way the window rehearsal tests already drive it with no person present: start-up to a usable window, opening an image, the marking screen's first paint, a pan and a zoom redraw, Accept and analyse to the analysis screen painted, opening a disclosure, and the Session records list with a few hundred sessions in it.
- **End to end:** one sheet from file to analysis, and a batch of ten.

### 3b. Every control, from the click to the moment it is finished

**Alan's second requirement:** the benchmark measures "the time it takes from clicking on a button, even one in the left bar, until the time the assets for that button are done loading, as well as all of the calculations and anything that needs to run to produce results."

**So the unit of measurement is a control, not a screen**, and the clock runs from the click to the moment the thing that click asked for is genuinely finished.

**Three times are recorded for every control, because they answer different complaints:**
- **To responsive:** the click is taken and the window accepts input again. This is the freeze a person feels.
- **To settled:** every piece of work that click started has finished. Background calculation, loading, rendering, reading from the database, waiting on a file, all of it. **Not the moment something appeared: the moment nothing further is coming.**
- **To final paint:** the screen shows its finished content, with no placeholder, no empty panel and no figure still to arrive.

**Record the work as well as the time:** how many layout and render passes the action caused, and whether any of it ran on the UI thread. A control that is fast because it painted nothing, or that is slow because it did its arithmetic on the UI thread, should be visible in the record as such.

**Every control, found by walking the window rather than by a list somebody maintains.** The rail's five destinations and the gear, every tool in the strip, every button in the header, every choice in a review item, every row in the shot table and the session list, every disclosure, every dropdown, the tabs of the print screen, and every button in a dialog.
- **A test walks the visual tree of every screen, collects everything a person can click, and fails when one of them has no bench case.** That is the same guard as section 3a's coverage test, one level finer, and it is what stops this rotting the next time a button is added.
- **A named exclusion list, short and justified in one line each**, for the controls that cannot run unattended: anything that prints to a device, opens a system file dialog, deletes data, or reaches the network. **Exclusion is by name in the record, never by silence.**
- **The worst controls go at the top of the record**, ordered by time to settled, because that list is the work queue for the phase.

**Where a case cannot be measured honestly**, say so in the record rather than leaving a blank or inventing a number.

### 4. What to build now

- **`grouplab bench`**, covering every case sections 3a and 3b list, each run enough times to be stable, reporting median and spread, and naming the machine, the build configuration and the commit.
- **The committed record**, one table, with the date and the machine it was measured on.
- **Stage timings already exist** in the stage record. Use them rather than inventing a second timing path.
- **A short `docs/PERFORMANCE.md`** holding the record, the method above, and the gate once it is written.
- **Nothing is optimised in this entry.** If you see something obviously wasteful while measuring, write it in the document as a candidate with its measurement beside it.

### 5. What Alan is noticing

**He says it is "somewhat slow running", and has not said where.** I have asked him. **Until he answers, do not guess which part he means:** measure all of section 2's operations, and the answer will probably name itself.

---

## 2026-09-20, entry 116: an installer and a zip somebody else can download and run, with no GitHub account and no .NET

**Status: actioned 2026-09-20**, sections 1 to 5. **Corrected 2026-09-21 by entry 120 section 9: the installer has been built.** Alan ran the release workflow by hand on commit 5a4cd07, and the draft release "GroupLab 0.1.0, draft" carries `grouplab-setup-win-x64.exe` and its versioned copy, identical by SHA-256, built on the GitHub Windows runner where Inno Setup is installed. It was never built on this machine, which is a different statement and is the one the original status line should have made. Question 29 was raised and nothing waited on it.
...
- **Section 2a:** an Inno Setup script, per user, no administrator rights, Start menu, Add or remove programs, an uninstall that leaves `%APPDATA%\GroupLab` alone and says so. `ISCC.exe` is not on this machine, so `-RequireInstaller` makes a release fail rather than ship without it; the installer itself was built on the runner when Alan ran the release by hand, and he is installing it.
