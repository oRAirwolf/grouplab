...
| 5.3.4 which bulls were aimed at | **done**: rows and columns presets, reachable from the shots per bull control. **the end to end proof against entry 120 is done**: scan 5's assignment matches Alan's table exactly, all twenty, and its zero correction across goes from 0.041 in, nothing to dial, to 1.163 in left. Scan 4 does not match and was never going to, needing entry 130 section 3.2's row breaks. Scan 6's nine detected holes are all on the right bull. Question 45 raised where scan 6 reads 9 holes and entry 130 recorded 10 |
