...
**Schema version 1.** `DESIGN.md` section 15: "Storage is SQLite with a documented schema and full JSON export." This is the document; `src/GroupLab.Core/Records/SessionStore.cs` creates the database, and `SessionSchemaTests` fails if the statements below differ from what it creates.
...
- **`rifles`, `barrels`, `loads`:** the person's records, keyed by name without regard to case, as a person refers to them. The solver's fields are all optional (`NOTES-FROM-PLANNING.md` entry 112 section 4). A record without them simply cannot use the solver.
