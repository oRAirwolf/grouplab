# What every published screenshot was made from

NOTES-FROM-PLANNING.md entry 126 section 3.2. **Everything in this folder is published on grouplab.org.** So it may only ever show material
...
## The sources that are allowed

| source | what it is |
|---|---|
| `Entry109Tests synthetic sheet` | A sheet GroupLab rendered and then composed holes into with `SyntheticSheet.Compose`, from a fixed seed. No camera, no scanner, no person's target. |
