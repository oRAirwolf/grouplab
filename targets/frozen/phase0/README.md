...
**If a future schema or validator change stops one of these loading or validating, that is a finding to report, not a fixture to repair.** `FrozenDefinitionTests` loads each file, checks that its identifier matches its file name, and validates it. It exempts test 26f alone: `GL-YCSK-DZZ1-R0VJ-4T5Y` has sighters outside its marker lattice by construction, which is the defect the geometry change of `docs/PHASE0-RESULTS.md` section 4.4 exists to fix.

| File | Live definition when printed | Printed as | Superseded by |
|---|---|---|---|
| `GL-YCSK-DZZ1-R0VJ-4T5Y.gltd.json` | `GL-CF25-LTR` | Sheets 1, 2 and 3, and the sheet printed at 96.2 percent | Not yet. The geometry change moves its sighter row; `docs/QUESTIONS-FOR-PLANNING.md` question 6 |
