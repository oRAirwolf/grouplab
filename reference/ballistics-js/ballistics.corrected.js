...
// website by itself: it is a file to look at, and to upload or not.
...
const BallisticSolver = (() => {
  ...
  function solve(params) {
    ...
    function runToRange(launchAngle, targetRangeYd) {
      ...
      // CORRECTED: the rifle is zeroed on the flat, as it is at a range, and the shooting angle is applied afterwards.
      let lvx = vBore * Math.cos(launchAngle);
      let lvy = vBore * Math.sin(launchAngle);
      let lt = 0;
    ...
    // CORRECTED: the muzzle is the sight height below the tilted line of sight, and the bore sits at the flat zero's angle above it.
    x  = sightHtFt * sinA;
    y  = -sightHtFt * cosA;
    ...
    while ((x * cosA + y * sinA) <= maxRangeFt + FT_PER_YD && t < 15) {
      // CORRECTED: range is measured along the line of sight and drop perpendicular to it, so a shooting angle no longer
  ...
  function coriolisVertical(latDeg, azDeg, vFps, tof) {
    ...
    // CORRECTED: firing east strikes high, so the deflection carries the sign of the acceleration rather than its opposite.
  ...
  function aeroJump(sg, crosswindFps, muzzleVel) {
    // CORRECTED: aerodynamic jump is not modelled. The formula below was not a published one and ran the wrong way with
    // stability, so it returns zero and the field stays in place for anything that reads it.
    return 0;
  ...
  function solveExtended(params) {
    ...
    return trajectory.map(row => {
      ...
      // CORRECTED: windDirDeg is the direction the wind comes from, so a wind from the right, a positive crosswind
      // component, pushes the bullet to the left. It used to add it as drift to the right.
