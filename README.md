...
- **In progress.** The stage timeline that shows the analysis doing its work. Built: each stage's record lands as it files during a live run, the timeline scrubs by slider or button, each stage shows its own picture once the analysis finishes (the markers found, the registration's corners ringed by their residual, the residual with the artwork gone), and a rejection clicked is found on the image. Not built: the live run `DESIGN.md` section 19 describes, where each picture appears as its stage lands, so the markers light up and the residual map settles while the analysis runs. What exists is a gallery at the end.
