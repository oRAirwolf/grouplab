...
**Test data.** Donated photographs live in a separate repository, [grouplab-testdata](https://github.com/oRAirwolf/grouplab-testdata), last checked against its commit `d35ef99`.
...
- **Every mark the application draws is legible on a photograph of a target.** A photograph is mostly white paper with black printing and coloured rings, sometimes over a dark backer.
  - **What that means:** no mark is drawn in white, and every stroke and label is drawn over a dark outline (entry 39 section 5). `src/GroupLab.App/Theme/Marks.cs` draws every mark that way.
- **No colour literal outside `src/GroupLab.App/Theme/Tokens.cs`.**
  - **How a colour is used instead:** every colour is a named token in two palettes, dark and light. A control takes its role from a class in `AppStyles`.
